...
def count_kmers(all_kmers):
    ...
    for kmer in all_kmers:
        if kmer not in kmer_dict:
            kmer_dict[kmer] = 1 #integer therefore make it 0
        else:
            kmer_dict[kmer] += 1
    
    kmer_count_dict = dict(sorted(kmer_dict.items(), key = lambda kmer_tuple: kmer_tuple[1], reverse = True))
...
def main():
    ...
    seq_list = seq_list_from_fastq_file(fastq_filename, kmer_length)
    
    for sequence in seq_list:
        kmer_per_sequence = sequence_to_kmer_list(sequence, kmer_length)
        all_kmers.extend(kmer_per_sequence)
